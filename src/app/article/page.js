...
export default function ArticlePage() {
  return (
    <main className="relative overflow-hidden bg-[#050505] text-[#f5e6a8]">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(255,0,149,0.18),transparent_38%),radial-gradient(circle_at_80%_20%,rgba(0,168,107,0.18),transparent_30%),linear-gradient(180deg,#050505_0%,#090909_100%)]" />
      <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col gap-14 px-6 py-10 sm:px-10 lg:px-12">
        <header className="flex flex-col gap-6 rounded-[2rem] border border-white/10 bg-white/5 p-6 backdrop-blur md:flex-row md:items-end md:justify-between">
          <div className="space-y-4">
            <p className="text-xs uppercase tracking-[0.45em] text-[#ff0095]">
              The Cherry Effect Journal
            </p>
            <h1 className="max-w-3xl text-4xl font-black uppercase leading-none tracking-[-0.05em] text-[#f5e6a8] sm:text-6xl">
              Sharp thinking for brands that want better decisions.
            </h1>
            <p className="max-w-2xl text-base leading-7 text-[#f5e6a8]/72 sm:text-lg">
          ...
          <Link
            href="/"
            className="inline-flex w-fit items-center gap-3 rounded-full border border-[#f5e6a8]/20 bg-[#f5e6a8]/8 px-5 py-3 text-sm font-semibold uppercase tracking-[0.2em] text-[#f5e6a8] transition hover:border-[#ff0095]/50 hover:bg-[#ff0095]/12 hover:text-white"
          >
            Back to Home
          </Link>
        </header>
        ...
        <section className="grid gap-6 md:grid-cols-3">
          {articleCards.map((article, index) => (
            <article
              key={article.title}
              className="group relative overflow-hidden rounded-[2rem] border border-white/10 bg-[#111111]/80 p-7 transition duration-500 hover:-translate-y-2 hover:border-[#ff0095]/50"
            >
              <div
              ...
              <div className="relative space-y-5">
                <span className="inline-flex rounded-full border border-white/10 px-3 py-1 text-[0.65rem] font-semibold uppercase tracking-[0.3em] text-[#f5e6a8]/72">
                  {article.category}
                </span>
                <h2 className="text-2xl font-black uppercase leading-tight tracking-[-0.04em] text-[#f5e6a8]">
                  {article.title}
                </h2>
                <p className="text-sm leading-7 text-[#f5e6a8]/72">
                  {article.excerpt}
                </p>
                <p className="text-xs uppercase tracking-[0.35em] text-[#00a86b]">
