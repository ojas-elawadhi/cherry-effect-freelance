...
export default function Hyperspeed({
  ...
  useEffect(() => {
    ...
    const distortions = {
      ...
      turbulentDistortion: {
        ...
        getJS: (progress, time) => {
          ...
          const getX = (value) =>
            ...
            Math.pow(
              Math.cos(Math.PI * value * uFreq.y + time * (uFreq.y / uFreq.x)),
              2,
            ) * uAmp.y;
          ...
          const getY = (value) =>
            ...
            Math.pow(
              nsin(Math.PI * value * uFreq.w + time / (uFreq.z / uFreq.w)),
              5,
            ) * uAmp.w;
      ...
      deepDistortion: {
        ...
        getJS: (progress, time) => {
          ...
          const getX = (value) =>
            Math.sin(value * Math.PI * uFreq.x + time) * uAmp.x;
          const getY = (value) =>
            Math.pow(value * uPowY.x, uPowY.y) +
            Math.sin(value * Math.PI * uFreq.y + time) * uAmp.y;
    ...
    class CarLights {
      ...
      init() {
        ...
        let lightColors = this.colors;
        if (Array.isArray(lightColors)) {
          lightColors = lightColors.map((color) => new THREE.Color(color));
        } else {
          lightColors = new THREE.Color(lightColors);
        }
        ...
        for (let i = 0; i < options.lightPairsPerRoadWay; i += 1) {
          ...
          offsetValues.push(laneX - carWidth / 2, offsetY, offsetZ);
          offsetValues.push(laneX + carWidth / 2, offsetY, offsetZ);

          metricValues.push(radius, length, speed);
          metricValues.push(radius, length, speed);

          const color = pickRandom(lightColors);
    ...
    class LightsSticks {
      ...
      init() {
        ...
        let stickColors = options.colors.sticks;
        if (Array.isArray(stickColors)) {
          stickColors = stickColors.map((color) => new THREE.Color(color));
        } else {
          stickColors = new THREE.Color(stickColors);
        }
        ...
        for (let i = 0; i < totalSticks; i += 1) {
          ...
          const color = pickRandom(stickColors);
          colorValues.push(color.r, color.g, color.b);
          metricValues.push(width, height);
        }
    ...
    options.distortion = distortions[distortionKey];
  ...
  return <div ref={hyperspeed} className="h-full w-full" />;
}
